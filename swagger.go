...
func CustomWrapHandler(config *Config, handler *webdav.Handler) flamego.Handler {
	...
	return func(ctx flamego.Context) {
		...
		if len(matches) != 3 {
			...
			_, _ = ctx.ResponseWriter().Write([]byte(http.StatusText(http.StatusNotFound)))
			return
		}
		...
		case "doc.json":
			...
			_, _ = ctx.ResponseWriter().Write([]byte(doc))
		default:
			handler.ServeHTTP(ctx.ResponseWriter(), ctx.Request().Request)
		}
	}
}
